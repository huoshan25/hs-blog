...
import { DialogueParser } from '@/modules/tts/config/dialogue-parser';
import { OssUploadService } from '@/modules/oss/ali/service/ossUpload.service';
import { TTSService } from '@/modules/tts/service/tts.service';
        ...
        messages: [
          {
            role: 'system',
            content:
              '你是一个专业的文章摘要生成助手。你需要生成不超过300个汉字的简短摘要，准确概括文章核心内容。',
          },
          {
            role: 'user',
            content: `
  ...
  async generateLongAudio(articleContent: string, articleUUID: string) {
    try {
      ...
      const dialogueContent =
        await this.generatePodcastDialogue(articleContent);
      ...
      // 3. 转换为多个音频Buffer
      const audioBuffers =
        await this.ttsService.generateDialogueSpeech(dialogueSegments);
